import { View, AnyBlock } from '@slack/bolt';
import { CALLBACK_IDS } from '../../handlers/modals/callbacks';
import { getAllZoomAccounts } from '../../config/zoomAccounts';
import { ZoomAccountConfig } from '../zoom/types';
 ...
 * アカウント選択ブロックを構築
 */
function buildAccountSelectBlock(accounts: ZoomAccountConfig[]): AnyBlock {
  return {
    type: 'input',
    ...
    element: {
      ...
      initial_option: {
        text: { type: 'plain_text', text: accounts[0].name },
        value: accounts[0].id,
      },
    },
    label: {
...
export class ModalBuilder {
  ...
  static buildZoomModal(privateMetadata: string): View {
    ...
    // アカウント選択（複数アカウントの場合のみ表示）
    if (accounts.length > 1) {
      blocks.push(buildAccountSelectBlock(accounts));
    }
